...
function solve(args) {
    ...
    for (var i = 0; i < el.length; i++) {
        oneLine = el[i].split(' ').filter(function (word) { return !!word; });
        //console.log(oneLine);
        if (oneLine.indexOf('#the-big-b{') == -1 && oneLine.indexOf('#the-big-b') > -1) {
            var check = oneLine[1];
    ...
    for (var j = 0; j < res.length; j++) {
        if (res[j] == '}') {
            res[j - 1] = res[j - 1].substring(0, res[j - 1].length - 1);
        }
    }
